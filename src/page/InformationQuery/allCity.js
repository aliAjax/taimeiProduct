...
export default class AllAirport extends Component{
    ...
    setScroll() {
        let airList = this.state.cityArr;
        let scrollObj={};
        let newArr=[];
        try{
            for(let i=0;i<airList.length;i++){
                newArr.push(airList[i].airportpinyin[0].toUpperCase())
            };
        }catch(e) {
    ...
    componentDidMount() {   // 加载渲染完成
        Axios({
            ...
            if(response.data.opResule == 0) {
                this.setState({
                    cityArr: response.data.list
                }, ()=>{
                    this.setScroll();
                })
            }else {
                    ...
                    <div className={style['item']} style={{height: 'calc(100% - 61px)', position: 'relative'}}>
                        <div>
                            <Breadcrumb separator="》" style={{fontSize: '1.6rem'}}>
                                ...
                                <Breadcrumb.Item>所有城市</Breadcrumb.Item>
                            </Breadcrumb>
                        </div>
                                    ...
                                    return (
                                        <div className={style['figure']} key={index} onClick={this.getInfo.bind(this, val)}>
                                            <div className={style['img']}>
                                                {
                                                    val.navBackground
                                                        ? <img src={`${val.navBackground}`} alt={val.cityname} />
                                                        : <img src={Slice} alt={val.cityname} />
                                                }
                                            </div>
                                            <div>
