...
export default class AllAirport extends Component{
    ...
    svgFn(val) {
        let a;
        companyIconData.forEach((v) => {
            if(v.companyIcao == val.icao) {
                a = <svg className={`${style['icon']} ${style['svg-logo']}`} aria-hidden="true">
    ...
    setScroll() {
        let airList = store.getState().companyList;
        let scrollObj={};
        let newArr=[];
        try{
            for(let i=0;i<airList.length;i++){
                newArr.push(airList[i].companyPinyin[0].toUpperCase())
            };
        }catch(e) {
    ...
    render(){
        ...
        return(
            <div className={'router-context'}>
                <div className={style['container']} style={{paddingTop: '61px', height: '100%'}}>
                    <div className={style['item']} style={{height: 'calc(100% - 61px)', position: 'relative'}}>
                        <div>
                            <Breadcrumb separator="》" style={{fontSize: '1.6rem'}}>
                                ...
                                <Breadcrumb.Item>所有航司</Breadcrumb.Item>
                            </Breadcrumb>
                        </div>
                        <div className={'scroll'} ref={'addScroll'} style={{maxHeight: '100%', overflowY: 'scroll'}}>
                            {
                                store.getState().companyList.map((val, index) => {
                                    return (
